...
public class ProblemSubmiting extends Composite implements IProblemSubmiting {
	...
	/**
	 * 
	 * DAMN DAMN DAMN!!! Workaround used to extract method from
	 * addProblemPropertiesFields()-method and still work with instantine-GWT
	 * plugin
	 * 
	 * all other tries to do it in simply way failed. I tried to extract
	 * non-static method in ProblemOverview - failed. I tried to extract static
	 * method in ProblemOverview - failed.
	 * 
	 * Obviously it worked normally (when executing application) but DESIGN-mode
	 * in instantine-GWT plugin blowed up (DAMN!)
	 * 
	 * @author michal
	 * 
	 */
	...
	VerticalPanel generateMiddlePanel() {
		...
		panel.add(StaticHelperClass.generateLabeledTextBoxPanel("Produkt:",
				_productListbox));
	...
	VerticalPanel generateAssignmentFieldsPanel() {
		VerticalPanel panel = new VerticalPanel();
		StaticHelperClass.setWidth("200");
		panel.add(StaticHelperClass.generateLabeledTextBoxPanel("Serwisant:",
				_servicemanTextBox));
		panel.add(StaticHelperClass.generateLabeledTextBoxPanel("Projektant:",
				_designerTextBox));
		panel.add(StaticHelperClass.generateLabeledTextBoxPanel("Programista:",
				_programmerTextBox));
		panel.add(StaticHelperClass.generateLabeledTextBoxPanel("Tester:",
				_testerTextBox));
		return panel;
	}
	...
	public Problem getFilledProblem() {
		...
		p.setProduct(_productListbox.getItemText(_productListbox
				.getSelectedIndex()));
		p.setClientImportance(_importanceTextBox.getItemText(_importanceTextBox
				.getSelectedIndex()));
